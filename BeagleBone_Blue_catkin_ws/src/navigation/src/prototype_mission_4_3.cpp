/*
* @file prototype_mission_4_3.cpp
* @Node name: prototype_mission_4_3
* @Publishing to:
*  All
...
*
* @description
* 	Prototype mission 4_3 with open loop controls
*
* @author | initials
...
* 	-
* @log
* 	11/14/2020: Created prototype_mission_4_3
* 1) Get into horizontal cruising configuration
* 2) Test 2 PWMs for 10 seconds each [1550, 1560]
*
*/
...
void MySigintHandler(int sig) {
  ROS_INFO("Shutting down: prototype_mission_4_3");
  ros::shutdown();
  return;
}
...
int main(int argc, char **argv) {
  ...
  ros::init(argc, argv, "prototype_mission_4_3");
  // create a handle of the node (name used in the file)
  ros::NodeHandle pm4_3;
  signal(SIGINT, MySigintHandler);

  //subscribers: vbs_dist, vbs_calibrated, beat
  ros::Subscriber vbs_cdist_sub = pm4_3.subscribe("vbs/vbs_current_dist", 1, vbs_cdistCallback);
  ros::Subscriber vbs_calibrated_sub = pm4_3.subscribe("vbs/vbs_calibrated", 1, vbs_calibratedCallback);
  ros::Subscriber heartbeat_sub = pm4_3.subscribe("heartbeat/beat", 1, beatCallback);
  ros::Subscriber leaking_sub = pm4_3.subscribe("heartbeat/leaking", 1, leakingCallback);

  //publishers
  ros::Publisher hfleft_ang_pub = pm4_3.advertise<std_msgs::Float32>("dtt/hfleft_ang", 1);
  ros::Publisher hfright_ang_pub = pm4_3.advertise<std_msgs::Float32>("dtt/hfright_ang", 1);
  ros::Publisher escleft_pub = pm4_3.advertise<std_msgs::Float32>("dtt/escleft", 1);
  ros::Publisher escright_pub = pm4_3.advertise<std_msgs::Float32>("dtt/escright", 1);
  ros::Publisher vbs_desired_dist_pub = pm4_3.advertise<std_msgs::Float32>("vbs/vbs_desired_dist", 1);
  ros::Publisher mm_ang_pub = pm4_3.advertise<std_msgs::Float32>("mm/mm_ang", 1);
  ros::Publisher vbs_calibrated_pub = pm4_3.advertise<std_msgs::Bool>("vbs/vbs_calibrated", 1);
