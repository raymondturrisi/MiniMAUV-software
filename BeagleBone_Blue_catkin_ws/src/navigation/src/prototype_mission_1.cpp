...
* 	-
* @log
* 	10/25/2020: Created prototype_mission_1
* 1) Center foils, calibrate vbs, center moving mass
* 2) move vbs to 50 mm
* 3) thrust forward for 20 seconds
* 4) shock reverse thrusters for assisted breaking, rotate mass, retract vbs to 10 mm
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber vbs_cdist_sub = pm1.subscribe("vbs/vbs_current_dist", 1, vbs_cdistCallback);
  ros::Subscriber vbs_calibrated_sub = pm1.subscribe("vbs/vbs_calibrated", 1, vbs_calibratedCallback);
  ros::Subscriber heartbeat_sub = pm1.subscribe("heartbeat/beat", 1, beatCallback);

  //publishers
  ros::Publisher hfleft_ang_pub = pm1.advertise<std_msgs::Float32>("dtt/hfleft_ang", 1);
  ros::Publisher hfright_ang_pub = pm1.advertise<std_msgs::Float32>("dtt/hfright_ang", 1);
  ros::Publisher escleft_pub = pm1.advertise<std_msgs::Float32>("dtt/escleft", 1);
  ros::Publisher escright_pub = pm1.advertise<std_msgs::Float32>("dtt/escright", 1);
  ros::Publisher vbs_desired_dist_pub = pm1.advertise<std_msgs::Float32>("vbs/vbs_desired_dist", 1);
  ros::Publisher mm_ang_pub = pm1.advertise<std_msgs::Float32>("mm/mm_ang", 1);
  ros::Publisher vbs_calibrated_pub = pm1.advertise<std_msgs::Bool>("vbs/vbs_calibrated", 1);
  ...
  while(ros::ok()) {
    ...
    //3) thrust forward for 20 seconds >> for ~~
    hfleft_ang.data = 180;
    hfright_ang.data = 180;
    escleft.data = 1600;
    escright.data = 1600;
    ...
    //4) shock reverse thrusters for assisted breaking, rotate mass, retract vbs to 10 mm >> while abs(cdist-ddist) > 5mm
    hfleft_ang.data = 180;
    hfright_ang.data = 180;
    escleft.data = 1600;
    escright.data = 1600;
